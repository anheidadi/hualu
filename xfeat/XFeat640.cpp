...
XFeat::XFeat(json laneData) {
    ...
    //刘沛远修改
    modelfile ="laneRegistration/weights/xfeat640.rknn";
...
void XFeat::run(const cv::Mat image, std::vector<cv::KeyPoint> &keys, cv::Mat &descs, int maxCorners) {
    ...
    ori_w=image.cols;
    //刘沛远修改
    width_scale=float(ori_w)/640.0;
    height_scale=float(ori_h)/640.0;
    cv::resize(image, imgs, cv::Size(640, 640),0,0, cv::INTER_LINEAR);
...
/*    float *M1=static_cast<float *>(outputs[0].buf);
    size_t element_count = outputs[0].size / sizeof(float);
    printf("element count %d\n",element_count);
...
/*    for (const auto& output : outputs) {
        float* data = static_cast<float*>(output.buf);
        size_t element_count = output.size / sizeof(float);
        std::cout << "Output[" << output.index << "]:" << std::endl;
        for (size_t j = 0; j < element_count; ++j) {
            std::cout << "Element[" << j << "] = " << data[j] << std::endl;
        }
    }*/
...
float XFeat::getOffset() {
    ...
    for(const auto i : result_set){
        ...
        double currentOffset=sqrt(pow((trans_x-stof(i[2])*scaleSize)*width_scale,2)+ pow((trans_y-stof(i[3])*scaleSize)*height_scale,2));
...
float XFeat::getLastOffset() {
    ...
    float maxOffset=0;
    int count=0;
    for(const auto i : last_result_set){
        ...
        float currentOffset=sqrt(pow((trans_x-last_result_set[count][2]*scaleSize)*width_scale,2)+
                pow((trans_y-last_result_set[count][3]*scaleSize)*height_scale,2));
        count++;
        if (currentOffset>maxOffset){
            maxOffset  = currentOffset;
...
int XFeat::isUpate(){
    ...
    if(minOffsetThresh<=offset<=maxOffsetThresh){
